# 后续开发指南

本文档提供iData AI创作平台后续开发的详细指引。

## 📋 开发计划

### 阶段一: 完成核心服务 (优先级: 高)

#### 1. 完成认证服务 (idata-auth)

**待实现:**
- [ ] AuthServiceImpl - 登录/注册/登出逻辑
- [ ] UserMapper - 用户数据访问
- [ ] User Entity - 用户实体类
- [ ] MyBatis XML映射文件

**参考代码:**

```java
// UserMapper.java
@Mapper
public interface UserMapper extends BaseMapper<User> {
    User selectByEmail(String email);
    User selectByPhone(String phone);
}

// AuthServiceImpl.java
@Service
public class AuthServiceImpl implements AuthService {
    @Autowired
    private UserMapper userMapper;
    
    @Autowired
    private JwtUtil jwtUtil;
    
    @Autowired
    private RedisService redisService;
    
    @Override
    @Transactional
    public LoginVO login(LoginDTO loginDTO) {
        // 1. 查询用户
        User user = userMapper.selectByEmail(loginDTO.getAccount());
        if (user == null) {
            user = userMapper.selectByPhone(loginDTO.getAccount());
        }
        
        // 2. 验证密码
        if (!PasswordUtil.matches(loginDTO.getPassword(), user.getPassword())) {
            throw new BusinessException(ErrorCode.PASSWORD_ERROR);
        }
        
        // 3. 生成Token
        String token = jwtUtil.generateToken(user.getId(), user.getUsername());
        
        // 4. 缓存到Redis
        redisService.setEx(RedisKeyConstants.getTokenKey(token), 
                          user.getId(), 
                          RedisKeyConstants.TOKEN_EXPIRE_TIME);
        
        // 5. 构造返回结果
        return buildLoginVO(user, token);
    }
}
```

#### 2. 开发系统管理服务 (idata-system)

**核心功能:**
- 用户管理 (CRUD)
- 角色管理
- 配置管理
- 操作日志

**POM文件:** 参考 idata-auth/pom.xml

**目录结构:**
```
idata-system/
├── src/main/java/com/idataai/system/
│   ├── SystemApplication.java
│   ├── controller/
│   │   ├── UserController.java
│   │   ├── RoleController.java
│   │   └── ConfigController.java
│   ├── service/
│   │   ├── IUserService.java
│   │   └── impl/UserServiceImpl.java
│   ├── mapper/
│   │   └── UserMapper.java
│   └── domain/
│       ├── entity/User.java
│       ├── dto/UserDTO.java
│       └── vo/UserVO.java
└── src/main/resources/
    ├── application.yml
    └── mapper/UserMapper.xml
```

#### 3. 开发应用服务 (idata-app)

**核心功能:**
- 应用模板列表
- 应用详情
- 创建实例
- 实例管理

**关键接口:**
```java
@RestController
@RequestMapping("/api/app")
public class ApplicationController {
    
    @GetMapping("/list")
    public Result<PageResult<ApplicationVO>> list(ApplicationQueryDTO queryDTO) {
        // 分页查询应用列表
    }
    
    @PostMapping("/instance/create")
    public Result<InstanceVO> createInstance(@RequestBody InstanceCreateDTO dto) {
        // 创建AI实例,发送到MQ异步处理
    }
}
```

**MQ消息处理:**
```java
@Component
public class InstanceConsumer {
    
    @RabbitListener(queues = RabbitMQConfig.AI_TASK_QUEUE)
    public void handleAiTask(InstanceCreateDTO dto) {
        // 1. 调用AI服务生成内容
        // 2. 保存结果
        // 3. 扣除积分
        // 4. 更新实例状态
    }
}
```

### 阶段二: 开发业务服务 (优先级: 中)

#### 4. 内容服务 (idata-content)
- 文章管理
- 思维分析

#### 5. 支付服务 (idata-payment)
- 会员购买
- 积分充值
- 订单管理

#### 6. 营销服务 (idata-marketing)
- 活动管理
- 邀请奖励
- 热点趋势

### 阶段三: AI集成与网关 (优先级: 中)

#### 7. AI服务 (idata-ai)

**集成OpenAI:**
```java
@Service
public class OpenAiService {
    
    @Value("${openai.api-key}")
    private String apiKey;
    
    public String generateText(String prompt) {
        // 调用OpenAI API
        // 使用 RestTemplate 或 WebClient
    }
}
```

#### 8. 网关服务 (idata-gateway)

**Spring Cloud Gateway配置:**
```yaml
spring:
  cloud:
    gateway:
      routes:
        - id: auth-service
          uri: lb://idata-auth
          predicates:
            - Path=/api/auth/**
        - id: app-service
          uri: lb://idata-app
          predicates:
            - Path=/api/app/**
```

## 🎨 前端开发

### 1. 创建布局组件

**DefaultLayout.vue:**
```vue
<template>
  <div class="layout">
    <el-container>
      <el-header>
        <Header />
      </el-header>
      <el-main>
        <router-view />
      </el-main>
      <el-footer>
        <Footer />
      </el-footer>
    </el-container>
  </div>
</template>
```

### 2. 开发核心页面

#### 登录页面 (views/auth/Login.vue)
```vue
<template>
  <div class="login-page">
    <el-form :model="loginForm" :rules="rules" ref="formRef">
      <el-form-item prop="account">
        <el-input v-model="loginForm.account" placeholder="邮箱/手机号" />
      </el-form-item>
      <el-form-item prop="password">
        <el-input v-model="loginForm.password" type="password" placeholder="密码" />
      </el-form-item>
      <el-button type="primary" @click="handleLogin">登录</el-button>
    </el-form>
  </div>
</template>

<script setup lang="ts">
import { ref } from 'vue'
import { useRouter } from 'vue-router'
import { useUserStore } from '@/stores/user'
import type { FormInstance } from 'element-plus'

const router = useRouter()
const userStore = useUserStore()
const formRef = ref<FormInstance>()

const loginForm = ref({
  account: '',
  password: ''
})

const handleLogin = async () => {
  await formRef.value?.validate()
  await userStore.login(loginForm.value)
  router.push('/')
}
</script>
```

#### 应用列表页面 (views/application/List.vue)
```vue
<template>
  <div class="app-list">
    <div class="app-grid">
      <AppCard 
        v-for="app in appList" 
        :key="app.id" 
        :app="app" 
        @click="handleAppClick(app)"
      />
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted } from 'vue'
import { getApplicationList } from '@/api/modules/application'
import AppCard from '@/components/business/AppCard/index.vue'

const appList = ref([])

const fetchList = async () => {
  const res = await getApplicationList({ current: 1, size: 20 })
  appList.value = res.data.records
}

onMounted(() => {
  fetchList()
})
</script>
```

### 3. 创建业务组件

#### AppCard组件 (components/business/AppCard/index.vue)
```vue
<template>
  <el-card class="app-card" @click="$emit('click')">
    <div class="app-icon">{{ app.appIcon }}</div>
    <div class="app-name">{{ app.appName }}</div>
    <div class="app-desc">{{ app.description }}</div>
    <div class="app-footer">
      <span>{{ app.creditsCost }} 积分</span>
      <span>{{ app.useCount }} 次使用</span>
    </div>
  </el-card>
</template>

<script setup lang="ts">
defineProps<{
  app: any
}>()

defineEmits<{
  (e: 'click'): void
}>()
</script>
```

## 🗄️ 完善数据库

### 添加完整的表结构

在 `database/schema/` 目录下创建:
- `02_content_tables.sql` - 内容相关表
- `03_payment_tables.sql` - 支付相关表
- `04_marketing_tables.sql` - 营销相关表
- `05_system_tables.sql` - 系统相关表

### 添加测试数据

在 `database/data/` 目录下创建:
- `01_test_users.sql` - 测试用户
- `02_test_applications.sql` - 测试应用
- `03_test_articles.sql` - 测试文章

## 🚀 部署配置

### Docker Compose完整配置

创建 `deployment/docker/docker-compose.yml`:

```yaml
version: '3.8'

services:
  mysql:
    image: mysql:8.0
    container_name: idata-mysql
    environment:
      MYSQL_ROOT_PASSWORD: root123
      MYSQL_DATABASE: idata_ai
    ports:
      - "3306:3306"
    volumes:
      - mysql-data:/var/lib/mysql
      - ../../database/schema:/docker-entrypoint-initdb.d
    networks:
      - idata-network

  redis:
    image: redis:7-alpine
    container_name: idata-redis
    ports:
      - "6379:6379"
    networks:
      - idata-network

  rabbitmq:
    image: rabbitmq:3.12-management
    container_name: idata-rabbitmq
    environment:
      RABBITMQ_DEFAULT_USER: admin
      RABBITMQ_DEFAULT_PASS: admin123
    ports:
      - "5672:5672"
      - "15672:15672"
    networks:
      - idata-network

  auth-service:
    build: ../../backend/idata-auth
    container_name: idata-auth
    ports:
      - "8081:8081"
    depends_on:
      - mysql
      - redis
    environment:
      SPRING_DATASOURCE_URL: jdbc:mysql://mysql:3306/idata_ai
      SPRING_REDIS_HOST: redis
    networks:
      - idata-network

  # 其他服务类似配置...

  nginx:
    image: nginx:alpine
    container_name: idata-nginx
    ports:
      - "80:80"
    volumes:
      - ../nginx/nginx.conf:/etc/nginx/nginx.conf
      - ../../frontend/dist:/usr/share/nginx/html
    depends_on:
      - auth-service
    networks:
      - idata-network

volumes:
  mysql-data:

networks:
  idata-network:
    driver: bridge
```

### Nginx配置

创建 `deployment/nginx/nginx.conf`:

```nginx
server {
    listen 80;
    server_name localhost;

    # 前端
    location / {
        root /usr/share/nginx/html;
        try_files $uri $uri/ /index.html;
    }

    # 后端API
    location /api/ {
        proxy_pass http://auth-service:8081/api/;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
    }
}
```

## ✅ 开发检查清单

### 后端服务检查

- [ ] 实体类继承BaseEntity
- [ ] Mapper继承BaseMapper<T>
- [ ] Service使用@Transactional
- [ ] Controller使用@RestController
- [ ] 统一异常处理
- [ ] 日志记录
- [ ] 参数校验(@Valid)
- [ ] Redis缓存
- [ ] 单元测试

### 前端开发检查

- [ ] TypeScript类型定义
- [ ] API封装
- [ ] 错误处理
- [ ] Loading状态
- [ ] 表单验证
- [ ] 响应式设计
- [ ] 代码格式化

## 📚 参考资源

- [Spring Boot官方文档](https://spring.io/projects/spring-boot)
- [MyBatis Plus文档](https://baomidou.com/)
- [Vue 3文档](https://vuejs.org/)
- [Element Plus文档](https://element-plus.org/)
- [TypeScript文档](https://www.typescriptlang.org/)

---

**提示**: 按照优先级逐步完成各个模块,先完成核心功能,再逐步扩展。
